...
public class BDVUtilities
{
	...
	//
	// /////////////////////////////////////////
	private static BigDataViewer bdv;

	public static BigDataViewer getBDVInstance() throws
                                               SpimDataException, IOException {
		if (bdv == null) {
			bdv = getEmptyBigDataViewer();
		}
		return bdv;
	}

	public static BigDataViewer getEmptyBigDataViewer() throws
                                                      SpimDataException, IOException {
		/*
		 * final String xmlFilenameOnly = "empty.xml"; final String h5FilenameOnly = "empty.h5";
		 * 
		 * 
		 * final byte[] xmlFile = SystemUtilities.getResourceFileContent(xmlFilenameOnly); final byte[] h5File =
		 * SystemUtilities.getResourceFileContent(h5FilenameOnly); ///Users/rhaase/temp/hdf5/
		 * 
		 * File tempDir = Files.createTempDir(); //new File(xmlFilename).
		 * 
		 * DebugHelper.print(new BDVUtilities(), "copy to " + tempDir +"/" + xmlFilenameOnly); //Files.copy(new File(xmlFilename), new File(tempDir + "/" +
		 * xmlFilenameOnly)); IOUtils.write(xmlFile, new FileOutputStream(tempDir + "/" + xmlFilenameOnly));
		 * 
		 * DebugHelper.print(new BDVUtilities(), "copy to " + tempDir +"/" + h5FilenameOnly); //Files.copy(new File(h5Filename), new File(tempDir + "/" +
		 * h5FilenameOnly)); IOUtils.write(h5File, new FileOutputStream(tempDir + "/" + h5FilenameOnly));
		 * 
		 * DebugHelper.print(new BDVUtilities(), "open " + tempDir + "/" + xmlFilenameOnly); return openFileUsingBigDataViewer(tempDir + "/" + xmlFilenameOnly,
		 * new boolean[] {false});
		 */
		final String xmlFilename = "/Users/rhaase/temp/hdf5/empty.xml";
		return openFileUsingBigDataViewer(xmlFilename, new boolean[] { false });

		// return openFileUsingBigDataViewer("/Users/rhaase/temp/hdf5/empty.xml", new boolean[] {false});
	}

	public static BigDataViewer openFileUsingBigDataViewer(String xmlFilename, boolean[] visibility) throws
                                                                                                   SpimDataException
  {
		// final BigDataViewer bdv = BigDataViewer.open( xmlFilename, "BDV", null );
		BigDataViewer
        bdv = BigDataViewer.open(xmlFilename, "BDV", null, new ViewerOptions());

		int idx = 0;
		// Set display mode to fused and make everything invisible -> Empty BDV
		final VisibilityAndGrouping vg = bdv.getViewer().getVisibilityAndGrouping();
		for (int i = 0; i < vg.numSources(); ++i) {
			if (i < visibility.length) {
				vg.setSourceActive(i, visibility[i]);
				if (visibility[i]) {
					idx = i;
				}
			} else {
				vg.setSourceActive(i, false);
			}
		}
		vg.setCurrentSource(idx);
		bdv.getViewer().getState().setCurrentSource(idx);
		vg.setDisplayMode(DisplayMode.FUSED);

		BDVUtilities.bdv = bdv;
	...
	public static void switchToXY(BigDataViewer bdv) {
		setCurrentTransform(bdv, getTransformToSwitchToPlane(bdv, 2));
	}

	public static void switchToYZ(BigDataViewer bdv) {
		setCurrentTransform(bdv, getTransformToSwitchToPlane(bdv, 0));
	}

	public static void switchToXZ(BigDataViewer bdv) {
	 ...
	 */
	public static boolean isXYView(BigDataViewer bdv) {
	...
	public static boolean isYZView(BigDataViewer bdv) {
		return isPlaneView(bdv, 0);
	}

	public static boolean isXZView(BigDataViewer bdv) {
		return isPlaneView(bdv, 1);
	}

	private static boolean isPlaneView(BigDataViewer bdv, int plane) {
	...
	private static AffineTransform3D getTransformToSwitchToPlane(
      BigDataViewer bdv, int plane) {
		// InitializeViewerState.initTransform()
		final Dimension dim = bdv.getViewer().getDisplay().getSize();
		...
		viewerTransform.translateTo(new double[] { -center[0], -center[1], -center[2] });
		if (plane == 0) // YZ
		{
		...
		// rotate
		viewerTransform.translate(center);
	 ...
	 */
	public static double[] getCenterTranslation(BigDataViewer bdv) {
		Interval sourceInterval;
		BoundingBoxModifierPlugin.MyBoundingBoxDialog bbd = BoundingBoxModifierPlugin.MyBoundingBoxDialog.getInstanceWithOutinitialisation();
		...
		if (bbd == null) {
          ...
          source = state.getSources().get(state.getCurrentSource()).getSpimSource();
			...
			sourceInterval = bbd.getCutSubvolume().getInterval();
		}
	...
	public static class BDVAffineTransform3D extends AffineTransform3D
  {
	 ...
	 */
	public static AffineTransform3D getCurrentTransform(BigDataViewer bdv) {
		if (bdv == null) {
			return null;
		}
		return bdv.getViewer().getDisplay().getTransformEventHandler().getTransform();
	}
	 ...
	 */
	public static void setCurrentTransform(BigDataViewer bdv, AffineTransform3D transform) {
		if (bdv == null) {
			return;
		}
	...
	public static Source<?> getCurrentSource(BigDataViewer bdv) {
		final ViewerState state = bdv.getViewer().getState();
	...
	public static JMenuItem addToBDVMenu(BigDataViewer bdv, String[] menuPosition, String title, ActionListener al) {
		//DebugHelper.print(new BDVUtilities(), "build menu");
		JMenuBar mb = bdv.getViewerFrame().getJMenuBar();
