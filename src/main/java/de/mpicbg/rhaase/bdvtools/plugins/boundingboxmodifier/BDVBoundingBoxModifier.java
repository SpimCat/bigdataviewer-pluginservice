package de.mpicbg.rhaase.bdvtools.plugins.boundingboxmodifier;
...
public class BDVBoundingBoxModifier implements MouseMotionListener {
	...
	private ModifiableInterval currentlyManipulatingInterval = null;
	...
	public BDVBoundingBoxModifier(BigDataViewer bdv) {
		this.bdv = bdv;
	}
	...
	public void mouseDragged(MouseEvent e) {
        ...
        display = bdv.getViewer().getDisplay();
		...
		if (x >= 0 && y >= 0 && x < display.getWidth() && y < display.getHeight()) {
			...
			bdv.getViewer().getState().getViewerTransform(at3d);
          ...
          currentInterval = BoundingBoxModifierPlugin.getCurrentlySelectedBoundingBoxRealInterval(bdv);
			...
			if (currentlyManipulatingInterval == null) {
				...
				double wy = target[1];
				double wz = target[2];
				distances[0] = Math.abs(currentInterval.min(0) - wx);
				distances[1] = Math.abs(currentInterval.max(0) - wx);
				distances[2] = Math.abs(currentInterval.min(1) - wy);
				distances[3] = Math.abs(currentInterval.max(1) - wy);
				distances[4] = Math.abs(currentInterval.min(2) - wz);
				distances[5] = Math.abs(currentInterval.max(2) - wz);
				distances[6] = Math.sqrt(Math.pow((currentInterval.min(0) + currentInterval.max(0)) / 2 - wx, 2)
								+ Math.pow((currentInterval.min(1) + currentInterval.max(1)) / 2 - wy, 2));
				distances[7] = Math.sqrt(Math.pow((currentInterval.min(1) + currentInterval.max(1)) / 2 - wy, 2)
								+ Math.pow((currentInterval.min(2) + currentInterval.max(2)) / 2 - wz, 2));
				distances[8] = Math.sqrt(Math.pow((currentInterval.min(0) + currentInterval.max(0)) / 2 - wx, 2)
								+ Math.pow((currentInterval.min(2) + currentInterval.max(2)) / 2 - wz, 2));
			...
			if (currentlyManipulatingInterval != null) {
				...
				long[] maxCoords = new long[3];
				ModifiableInterval
            currentInterval2 = BoundingBoxModifierPlugin.getCurrentlySelectedBoundingBoxRealInterval(bdv);
				currentInterval2.min(minCoords);
				currentInterval2.max(maxCoords);
				...
				long width = maxCoords[0] - minCoords[0];
				long height = maxCoords[1] - minCoords[1];
				long depth = maxCoords[2] - minCoords[2];
				...
				System.out.println(
								"new box: (x,y,z) "
												...
												+ " (w,h,d) "
												+ Arrays.toString(new long[] { maxCoords[0] - minCoords[0], maxCoords[1] - minCoords[1],
