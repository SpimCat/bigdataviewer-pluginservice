package de.mpicbg.rhaase.bdvtools.plugins.importexport;
...
import de.mpicbg.rhaase.utilities.ColorUtilities;
import de.mpicbg.rhaase.utilities.ImageJUtilities;
import de.mpicbg.rhaase.utilities.ImgLib2Utils;
																					 ...
																					 SupportsBigDataViewerToolBarButton
{
	...
	@Override public int addToolBarButtons(int verticalPosition)
	{
		...
		bdv.getViewer().addRenderTransformListener(this);
	...
	public static void importToSelectedSource(BigDataViewer bdv, ImagePlus imp, double samplingFactor) {
	...
	public static void importLabelMapToSelectedSource(BigDataViewer bdv, ImagePlus imp, double samplingFactor) {
		
		ImportFromIJPlugin.ImportImagePlusToBDVinCurrentBoundingBox(bdv, imp.getTitle(), imp, null, true, samplingFactor);
		
	}
		
	public static void ImportImgToBDV(BigDataViewer bdv, String title, RandomAccessibleInterval<UnsignedShortType> img, RealARGBColorConverter<UnsignedShortType> converter,
	...
	{	
		...
		boxConverterSetup.setViewer(bdv.getViewer());
		...
		// Put in BDV
		bdv.getViewer().addSource(boxSourceAndConverter);
		bdv.getSetupAssignments().addSetup(boxConverterSetup);

		final int bbSourceIndex = bdv.getViewer().getState().numSources() - 1;
	...
	public static void ImportImgToBDVinCurrentBoundingBox(BigDataViewer bdv, String title, Img<UnsignedShortType> img, double[] voxelSize, ARGBType color,
		...
		if (interval != null) {
			transform.translate(new double[] { interval.min(0) * voxelSize[0], interval.min(1) * voxelSize[1], interval.min(2) * voxelSize[2] });
		}
		...
		int formerCurrentSource = bdv.getViewer().getState().getCurrentSource();
		...
		if (keepView && interval != null) {
			bdv.getViewer().getVisibilityAndGrouping().setCurrentSource(formerCurrentSource);
			// bdv.getViewer().getState().setCurrentSource(formerCurrentSource);
			BDVUtilities.setCurrentTransform(bdv, formerViewTransform);
		}
	}

	public static void ImportImagePlusToBDV(BigDataViewer bdv, String title, ImagePlus imp, ARGBType color, AffineTransform3D sourceTransform) {
	...
	public static void ImportImagePlusToBDVinCurrentBoundingBox(
      BigDataViewer bdv, String title, ImagePlus imp, ARGBType color, boolean keepView,
